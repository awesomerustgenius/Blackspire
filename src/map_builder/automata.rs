...
impl MapArchitect for CellularAutomataArchitect {
    fn new(&mut self, rand: &mut RandomNumberGenerator) -> MapBuilder {
        ...
        self.random_noise_maps(rand, &mut mb.map);
        for _ in 0..10 {
            self.iteration(&mut mb.map);
        }

        let start = self.find_start(&mb.map);
        mb.monster_spawns = mb.spawn_monster(&start, rand);
...
impl CellularAutomataArchitect {
    fn random_noise_maps(&mut self, rand: &mut RandomNumberGenerator, map: &mut Map) {
    ...
    fn find_start(&mut self, map: &Map) -> Point {
        ...
        let closest_point = map
            ...
            .min_by(|(_, dist_1), (_, dist_2)| dist_1.partial_cmp(dist_2).unwrap())
            .map(|(idx, _)| idx)
            .unwrap();
